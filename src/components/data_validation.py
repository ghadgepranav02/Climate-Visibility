...
class DataValidation:
    ...
    def validate_file_name(self,file_path:str) -> bool:
        ...
        try:
            ...
            regex = r"^visibility_\d{8}_\d{6}\.csv$"
            if re.match(regex, file_name):
                splitAtDot = file_name.replace('.csv', '').split('_')
                filename_validation_status = (
                    len(splitAtDot[1]) == LENGTH_OF_DATE_STAMP_IN_FILE
                    and len(splitAtDot[2]) == LENGTH_OF_TIME_STAMP_IN_FILE
                )
            else:
